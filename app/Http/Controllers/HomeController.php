...
class HomeController extends Controller
{
    public function index(){
       ...
       if(Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%about%")->where('page_type','Group')->latest()->first()!=null){
        ...
        if(Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%projectupdates%")->where('page_type','Group')->latest()->first()!=null){
            $projectupdates_id = Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%projectupdates%")->where('page_type','Group')->latest()->first()->id;
            $projectupdates = Navigation::query()->where('parent_page_id',$projectupdates_id)->latest()->get();
        }
        else{
            $projectupdates = null;
        } 
        ...
        if(Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%blog%")->where('page_type','Group')->latest()->first()!=null){
            $blog_id = Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%blog%")->where('page_type','Group')->latest()->first()->id;
            $blog = Navigation::query()->where('parent_page_id',$blog_id)->latest()->get();
        }
        else{
            $blog = null;
        } 
        ...
        //return $missons;       
        return view("website.index")->with([ 'newsevents'=>$newsevents,'projectlist'=>$projectlist,'blog'=>$blog,'projectupdates'=>$projectupdates,'testimonial'=>$testimonial,'statistics'=>$statistics,'banners'=>$banners,'partners'=>$partners,'about'=>$About,'menus'=>$menus,'global_setting'=>$global_setting,'sliders'=>$sliders,'missons'=>$missons,'message'=>$message,'jobs'=>$jobs]);
    }
    ...
    public function category($menu){
        ...
        elseif($category_type == "Group Project"){
            ...
            return view("website.newsevents")->with(['newsevents'=>$eventlist, 'projectlist'=>$projectlist, 'normal'=>$normal,'jobs'=>$jobs,'menus'=>$menus,'sliders'=>$sliders,'about'=>$About,'global_setting'=>$global_setting,'slug1'=>$slug1,'slug2'=>$slug2]);
